...
const OrderModal = ({ open, onClose, order }) => {
  ...
  const getStatusBadge = (status) => {
    const statusMap = {
      ...
      completed: { text: "Hoàn thành", color: "bg-teal-100 text-teal-800" },
  ...
  const getPaymentStatusText = (status) => {
    if (status === "paid") return <span className="text-green-600">Đã thanh toán</span>;
    if (status === "refunded") return <span className="text-purple-600">Đã hoàn tiền</span>;
    return <span className="text-yellow-600">Chưa thanh toán</span>;
  };
